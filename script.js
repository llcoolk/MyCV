$(function() {
  ...
  $.getJSON("./resume.json", function(result) {
    ...
    $.each(result.basics.profiles, function(i, profile) {
      profilesOutput += `<li><a href="${profile.url}">${socialIcon(
        profile.network
      )}</a></li>`;
    });
    ...
    $.each(result.languages, function(i, lang) {
      langOutput += `<img src="http://www.countryflags.io/${
        lang.code
      }/shiny/32.png">`;
    });
    ...
    $.each(result.volunteer, function(i, vol) {
      volOutput += `
            <li class="mb-5">
                ...
                <h5 class="label">${vol.position}</h5>
            ...
            <div class="col-md-6">
                <div class="card mb-4">
                    <div class="card-body">
                        ...
                        <a href="${
                          repo.html_url
                        }" class="btn btn-info" target="_blank">See repo</a>
                    </div>
                </div>
            </div>
            `;
